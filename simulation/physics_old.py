...
class vector3:
    ...
    def angleBetweenVectors(self, vector: vector3) -> float:
        if isinstance(vector, vector3):
...
class Quaternion:
    ...
    def eulerToQuaternion(self, eulerAngles : vector3 ) -> Quaternion:

        cr = math.cos(eulerAngles.x / 2)
        cp = math.cos(eulerAngles.y / 2)
        cy = math.cos(eulerAngles.z / 2)

        sr = math.sin(eulerAngles.x / 2)
        sp = math.sin(eulerAngles.y / 2)
        sy = math.sin(eulerAngles.z / 2)
...
class TVC:
    ...
    def calculateForces(self, thrust):
        ...
        self.force.x = thrust * math.cos(self.position.y) - (thrust - ( thrust * math.cos(self.position.z) ) )
