...
import numpy as np
def load_motor_file(file_name) -> list:
...
class rocket_motor:
    ...
    def add_motor(self, motor_name: str, motor_path):
        motor_data_raw = load_motor_file(motor_path)
        motor_data_interpolated = interpolateThrust(motor_data_raw, self.time_step)
        ...
        for motor in self.motors:
            if self.motors[motor]["is_lit"] == True:
                if time_int - self.motors[motor]["ignition_time"] >= len(self.motors[motor]["data"]) or time_int - self.motors[motor]["ignition_time"] < 0:
                    ...
                    # if time_int - self.motors[motor]["ignition_time"] > 3000 or time_int - self.motors[motor]["ignition_time"] < 500:
